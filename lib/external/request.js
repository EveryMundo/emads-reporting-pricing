...
const getOrganizationSettings = async (tenantCode) => {
  const URL = `${process.env.OPENAIR_HOST}/organization-service/v1/tenants/code/${tenantCode.toLowerCase()}/settings/display-default`
  logr.debug(URL)
  const endpoint = httpClient.urlToEndpoint(URL, {
    'EM-API-Key': process.env.EM_BE_API_KEY,
    'content-type': 'application/json',
